...
export default function MyRunsPage() {
  ...
  const { data, pending, error } = useUnit({
    data: runsModel.my.$data,
    pending: runsModel.my.$pending,
    error: runsModel.my.$error,
  })
  ...
  return (
    <div className="p-6">
      <PageHeader title="My Runs" description="History of your assistant runs" />
      ...
      {!pending && data && data.runs.length === 0 && (
        <EmptyState
          ...
          title="No runs yet"
          description="Run an assistant to see your history here"
        />
      )}
        ...
        <>
          {error && (
            <ErrorBlock
              ...
              onRetry={() => runsModel.my.fx({ page, pageSize, status })}
            />
          )}
