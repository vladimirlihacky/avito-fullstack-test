...
export function useUpdateAssistant(
  ...
  return useMutation({
    ...options,
    mutationFn: ({ assistantId, input }) => assistantsApi.update(assistantId, input),
    onSuccess: (data, variables, onMutateResult, context) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.assistants.lists() });
      queryClient.setQueryData(queryKeys.assistants.detail(variables.assistantId), data);
      options?.onSuccess?.(data, variables, onMutateResult, context);
    },
  });
}
...
export function useRunAssistant(
  ...
  return useMutation({
    ...options,
    mutationFn: ({ assistantId, input }) => assistantsApi.run(assistantId, input),
    onSuccess: (data, variables, onMutateResult, context) => {
